...
const Inicio = ({ data = [] }) => {
  ...
  const manejarEditarReporte = (reporte) => {
    window.open(`/reports/${reporte._id}`, "_blank", "width=1200,height=820,resizable=yes");
    setSelectedNote(null);
  };

  const manejarSeleccionarPlantilla = (plantilla) => {
    if (!selectedNote) return;
    window.open(`/reports/${selectedNote._id}/${plantilla._id}`, "_blank", "width=1200,height=820,resizable=yes");
    setSelectedNote(null);
  };

  
  const datosFormateadosParaLista = data.map((paciente) => ({
    id: paciente._id,
  ...
  const plantillasFiltradasYMapeadas = plantillas
    .filter((p) =>
    ...
    <div className="w-full">
      ...
      <div className="max-w-[1540px] mx-auto mt-16 p-4 flex flex-col gap-4">
        <div className="flex flex-wrap gap-4 justify-end">
          <Link to="/CreatePage" className="btn btn-soft btn-primary font-bold text-[1.1em] flex gap-2">
            <PlusIcon size={18} /> Agregar Nueva Plantilla
          </Link>
          <Link to="/editPlantilla" className="btn btn-soft btn-primary font-bold text-[1.1em] flex gap-2">
            <Pencil size={18} /> Editar Plantilla
          </Link>
        </div>

        <h1 className="font-bold text-2xl lg:text-3xl my-2">Lista de Reportes</h1>
        
        
        <List 
          data={datosFormateadosParaLista} 
          onActionClick={(pacienteOriginal) => setSelectedNote(pacienteOriginal)}
          textoBoton="Reporte"
          iconoBoton={BookText}
        />
      </div>
